...
export async function addTransaction(
  ...
  // Sync ke budget — kalau user pilih budget tertentu (overrideBudgetId),
  // masukkan ke sana. Kalau tidak, match otomatis berdasarkan bulan transaksi.
  if (
...
export async function getAccounts(): Promise<AccountWithBalance[]> {
  ...
  const [{ data: accounts, error: accError }, { data: txs, error: txError }] =
    await Promise.all([
      supabase
        .from("accounts")
        .select("*")
...
async function syncBudgetActual(
  ...
  if (overrideBudgetId) {
    // User sudah pilih mau masuk ke budget mana — langsung target budget itu
    ...
    // Default: cari budget yang bulan/tahunnya cocok dengan tanggal transaksi
    const { data } = await supabase
      .from("budgets")
      .select("id, period, year, month, week")
      .eq("user_id", userId)
      .eq("year", year)
      .or(
        `and(period.eq.monthly,month.eq.${month}),and(period.eq.weekly,month.eq.${month},week.eq.${week})`,
      );
    budgets = data;
  }
  ...
  for (const budget of budgets) {
    ...
    const totalActual = (allCats ?? []).reduce(
      (s, c) => s + Number(c.actual_amount),
      0,
    );
...
export async function updateTransaction(
  ...
  if (
    original &&
    (original.type === "expense" || original.type === "debt_payment") &&
  ...
  if (
    (data.type === "expense" || data.type === "debt_payment") &&
    data.category?.name
  ) {
    await syncBudgetActual(
      userId,
...
export async function updateGoalAmount(id: string, amount: number) {
  ...
  if (existing && amount >= Number(existing.target_amount))
...
export async function getSavingsOverview(): Promise<SavingsOverview> {
  ...
  const [
    ...
    supabase
      .from("wishlists")
      .select("id, name, saved_amount, price")
      .eq("user_id", userId)
      .eq("status", "pending"),
  ]);
  ...
  const items: SavingsOverviewItem[] = [
    ...
    ...(wishlist ?? []).map((w) => ({
      ...
      saved: Number(w.saved_amount),
      target: Number(w.price),
    })),
...
export async function getDashboardStats() {
  ...
  const prev_monthly_income = prevTransactions
    .filter((t) => t.type === "income")
    .reduce((s, t) => s + Number(t.amount), 0);
  const prev_monthly_expense = prevTransactions
    .filter((t) => t.type !== "income" && t.type !== "transfer")
    .reduce((s, t) => s + Number(t.amount), 0);
  ...
  const income_trend =
    prev_monthly_income > 0
      ? Math.round(
          ((monthly_income - prev_monthly_income) / prev_monthly_income) * 1000,
        ) / 10
      : null;
  const expense_trend =
    prev_monthly_expense > 0
      ? Math.round(
          ((monthly_expense - prev_monthly_expense) / prev_monthly_expense) *
            1000,
        ) / 10
      : null;

  const total_active_debt = debts.reduce((s, d) => s + Number(d.remaining), 0);
  const total_debt_amount = debts.reduce(
  ...
  const total_active_receivable = receivables.reduce(
  ...
  const nearest_due =
    [...debts].sort((a, b) => {
      const dateA =
        a.is_installment && a.next_due_date ? a.next_due_date : a.due_date;
      const dateB =
        b.is_installment && b.next_due_date ? b.next_due_date : b.due_date;
  ...
  return {
    ...
    debt_paid_percentage:
      total_debt_amount > 0
        ? Math.round((total_debt_paid / total_debt_amount) * 100)
        : 0,
    health_score: null,
    nearest_due,
  };
...
export async function getForecastDefaults() {
  ...
  const totalExpense = transactions
    ...
    .reduce((s, t) => s + Number(t.amount), 0);
  const totalDebtPayment = transactions
...
export async function updateBudget(
  ...
  const { categories, ...budgetData } = budget;
  const total_planned = categories.reduce((s, c) => s + c.planned_amount, 0);
  const total_actual = categories.reduce(
    (s, c) => s + (c.actual_amount ?? 0),
    0,
  );
...
export async function getNotifications(): Promise<Notification[]> {
  ...
  const storedNotifs = ((stored ?? []) as Notification[]).filter(
    (n) => debtDueEnabled || n.type !== "debt_due",
  );
  ...
  const liveNotifs: Notification[] = !debtDueEnabled
    ? []
    : (debts ?? [])
        .map((d): Notification | null => {
          const dueStr =
            d.is_installment && d.next_due_date ? d.next_due_date : d.due_date;
...
export async function globalSearch(query: string): Promise<SearchResult[]> {
  ...
  const [debts, transactions, goals, wishlist, receivables, accounts] =
    await Promise.all([
      supabase
        .from("debts")
        .select("id, name, lender")
        .eq("user_id", userId)
        .ilike("name", like)
        .limit(5),
      supabase
      ...
      supabase
        .from("receivables")
        .select("id, name, borrower, total_amount")
        .eq("user_id", userId)
        .ilike("name", like)
        .limit(5),
      supabase
  ...
  (transactions.data ?? []).forEach((t) =>
    results.push({
      ...
      type: "transaction",
      title: t.name,
      subtitle: `Transaksi • Rp ${Number(t.amount).toLocaleString("id-ID")}`,
      href: "/transactions",
    }),
  );
  (goals.data ?? []).forEach((g) =>
    results.push({
      ...
      type: "goal",
      title: g.name,
      subtitle: `Tujuan • Rp ${Number(g.target_amount).toLocaleString("id-ID")}`,
      href: "/goals",
    }),
  );
  (wishlist.data ?? []).forEach((w) =>
    results.push({
      ...
      type: "wishlist",
      title: w.name,
      subtitle: `Wishlist • Rp ${Number(w.price).toLocaleString("id-ID")}`,
      href: "/wishlist",
    }),
  );
