...
export function DebtPaymentModal({ debt, open, onClose }: Props) {
  const recordDebtPayment = useAppStore((s) => s.recordDebtPayment);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (val > debt.remaining) {
      toast.error(`Melebihi sisa utang (${formatCurrency(debt.remaining)})`);
      return;
    }

    setLoading(true);
    await new Promise((r) => setTimeout(r, 500));
    recordDebtPayment(debt.id, val);
