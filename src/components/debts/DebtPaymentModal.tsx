...
import { X, Loader2, CreditCard, Calendar, Repeat, BookOpen } from "lucide-react";
import { toast } from "sonner";
import { formatCurrency, formatDate, formatInputNumber, parseInputNumber } from "@/utils";
import { addTransaction, getCategories, getAccounts, getBudgets } from "@/lib/db";
...
export function DebtPaymentModal({ debt, open, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    setNotes(""); setCategoryId(""); setAccountId(""); setOverrideBudgetId("");
  }, [open, debt]);
  ...
  useEffect(() => {
    if (open) getBudgets().then((data) => setBudgets(data as Budget[])).catch(() => { });
  }, [open]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Nama transaksi menyertakan nama utang lengkap supaya keyword_filter
      // di budget category bisa auto-match (mis. "Bayar Utang: KPR BRI" → keyword "KPR")
      const txName = installmentNum
        ? `${installmentNum} ${debt.name}`
        : `Bayar Utang: ${debt.name}`;
      ...
      setAmountRaw(""); setAmountDisplay(""); setNotes(""); setCategoryId(""); setAccountId(""); setOverrideBudgetId("");
      onClose();
  ...
  const expenseBudgets = budgets.filter((b) => b.period === "monthly" || b.period === "weekly");
  const selectedBudget = expenseBudgets.find((b) => b.id === overrideBudgetId);

  function budgetLabel(b: Budget) {
    if (b.period === "monthly" && b.month) return `${b.name} · ${MONTHS[(b.month ?? 1) - 1]} ${b.year}`;
    return b.name;
  }

  const inputClass = "w-full bg-input border border-border rounded-md px-3 py-2.5 text-sm text-text-primary placeholder:text-accent focus:outline-none focus:border-accent transition-colors";
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-sm bg-surface-card border border-border rounded-t-2xl sm:rounded-xl shadow-2xl animate-fade-in max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            {isInstallment && totalCount > 0 && (
              <div className="pt-2 border-t border-border">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs font-semibold text-text-primary tabular-nums">{paidCount} / {totalCount} bulan</span>
                </div>
                <div className="w-full h-1.5 bg-border rounded-full overflow-hidden">
                  <div className="h-full bg-text-primary rounded-full transition-all" style={{ width: `${Math.min(100, (paidCount / totalCount) * 100)}%` }} />
                </div>
                {debt.next_due_date && (
                  <div className="flex items-center gap-1 mt-1.5 text-[10px] text-accent">
                    <Calendar size={9} /> Cicilan berikutnya: {formatDate(debt.next_due_date)}
                  </div>
                )}
              </div>
            )}
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className={labelClass} style={{ marginBottom: 0 }}>Jumlah Bayar *</label>
              {isInstallment && debt.installment_amount && (
                <button type="button" onClick={() => fillAmount(Number(debt.installment_amount))}
            ...
            <div className="relative mt-1.5">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-accent text-sm font-medium">Rp</span>
              <input
                inputMode="numeric"
                value={amountDisplay}
            ...
            {amount > 0 && (
              <p className="text-[11px] text-text-secondary mt-1 tabular-nums">= {formatCurrency(amount)}</p>
            )}
            {/* Quick fill buttons */}
            <div className="flex gap-2 mt-2">
              {(isInstallment
                ...
                <button key={label} type="button" onClick={() => fillAmount(val)}
          ...
          {expenseBudgets.length > 0 && (
            <div>
              ...
              <select value={overrideBudgetId} onChange={(e) => setOverrideBudgetId(e.target.value)}
              ...
              {selectedBudget && (
                <p className="text-[10px] text-warning mt-1.5">
                  ⚡ Dipotong dari budget <span className="font-semibold">{selectedBudget.name}</span>.
                </p>
              )}
            </div>
          )}
          ...
          {/* Akun */}
          <div>
            <label className={labelClass}>Bayar dari Akun <span className="text-accent">(opsional)</span></label>
          ...
          {/* Kategori budget */}
          <div>
            <label className={labelClass}>Kategori Budget <span className="text-accent">(opsional)</span></label>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button type="submit" disabled={loading || !amountRaw}
